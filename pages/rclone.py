...
def exec_start(RCLONENAME, RCLONE_CONFIG, FLASKTIME, RCLONE_LOCAL, RCLONE_REMOTE,RCLONE_C_M, RCLONE_include):
	if RCLONE_C_M == 'move':
		FLASKAPPS = '/data/rclone ' + RCLONE_C_M + ' ' + RCLONE_LOCAL + ' ' + RCLONE_REMOTE + ' -L --config ' + RCLONE_CONFIG + ' ' + RCLONE_include + ' --log-level INFO --min-age 1m --stats 10s --stats-file-name-length 0 --min-age 1m --log-file /data/log/rclone.log --transfers=4 --checkers=8 --delete-empty-src-dirs --create-empty-src-dirs --delete-after --drive-chunk-size=1M'
	else:
		FLASKAPPS = '/data/rclone ' + RCLONE_C_M + ' ' + RCLONE_LOCAL + ' ' + RCLONE_REMOTE + ' -L --config ' + RCLONE_CONFIG + ' ' + RCLONE_include + ' --log-level INFO --min-age 1m --stats 10s --stats-file-name-length 0 --min-age 1m --log-file /data/log/rclone.log --transfers=4 --checkers=8 --create-empty-src-dirs --delete-after --drive-chunk-size=1m'
	print(FLASKAPPS)
